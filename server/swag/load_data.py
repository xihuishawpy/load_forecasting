...
while (dt.date() != end_date):
	...
	day = dt.date().day
	link = (
	    f"static/SLDC_Data/{str(year)}/{str(month).zfill(2)}/" + str(day).zfill(2)
	    + "-" + str(month).zfill(2) + "-" + str(year) + ".csv")
	try:
		print(day,month,year);
		dataReader = csv.reader(open(link), delimiter=',', quotechar='"')
		for ip, row in enumerate(dataReader):
			print("q",ip)
			sp = row[0].split(':')
			...
			while (tim != dt.time()):
				print(tim, (dt.time()))
				print("w")
				results = [CSV.objects.filter(date = dt.date(), timestamp = str(dt.time()))]
				if (len(results[0]) == 0):
